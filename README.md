...
- `封箱单 批次号 日期.xlsx`：以 `t.xlsx` 为模板，每个箱号一张工作表；箱号超过模板工作表数量时自动复制工作表，不设 42 箱限制。只写入 A/B/C，逐件展开，D 不进入封箱单。
- `入库质检报告 批次号 日期.xlsx`：以 `入库质检报告 0715.xlsx` 为模板，仅保留 A/B/C/D 等级表和 `瑕疵照片 Sample`。
